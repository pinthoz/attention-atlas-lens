...
Why Attention Atlas Lens is built the way it is. Setup and usage live in the
[README](../README.md); this file is the reasoning behind the decisions, kept
because most of them are easy to undo by accident.

## The API it expects

This frontend talks to the JSON API in the `attention-atlas` repository
(`service/api.py`, run with `uvicorn service.api:api --port 8000`). It uses
four endpoints: `GET /api/health`, `GET /api/models`, `POST /api/analyze` and
`POST /api/bias`.
...
The API is live on the Space at `https://pinthoz-attention-atlas.hf.space/api/*`,
served by the same container as the Shiny dashboard, which stays mounted at `/`.
